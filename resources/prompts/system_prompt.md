...
- **Free text** (prose, an event/topic `summary`, a message, anything you save): **spell the month as a word — always.** Write `June 4, 2026` or `4 June 2026`, never a bare numeric date. This is your strong default everywhere you talk to the user or persist a date, because a worded month cannot be misread as day-first or month-first — it sidesteps the whole ambiguity, so it needs **no** format tag. Times still follow the clock's format (`2:30 PM` or `14:30`). Only if you have a real reason to write a numeric date instead, note the format right after it — `06/04/2026 (MM/DD)` — so it stays unambiguous.
